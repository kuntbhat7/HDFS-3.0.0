...
fix_spark_scripts() {
    sed -i 's~/usr/hdp/${HDP_VERSION}~/usr/local~g' ${SPARK_HOME}/bin/spark-class
}
...
set_env_variables() {
  ...
  CLASSPATH="$PIPELINE_CONFIG:$SPARK_LIB_HOME/*:$PIPELINE_LIB/*:${HADOOP_HOME}/share/hadoop/yarn/lib/*"
}
